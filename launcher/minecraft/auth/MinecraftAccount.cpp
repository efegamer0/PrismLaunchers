...
MinecraftAccountPtr MinecraftAccount::createOffline(const QString& username)
{
    ...
    // Offline hesabı doğrudan hazır ve doğrulanmış moda sokuyoruz
...
shared_qobject_ptr<AuthFlow> MinecraftAccount::refresh()
{
    ...
    // Offline hesap yenilenmeye çalışıldığında sunucuya gitmesini engelle
    if (data.type == AccountType::Offline) {
        data.accountState = AccountState::Online;
...
{
    // Offline hesapların yenileme kontrolü pas geçilir
    if (data.type == AccountType::Offline) {
        return false;
    }
...
void MinecraftAccount::fillSession(AuthSessionPtr session)
{
    ...
    if (!session->access_token.isEmpty()) {
        session->session = "token:" + session->access_token + ":" + session->uuid;
    } else {
        session->session = "-";
    }
}
